...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cards.forEach(card => {
        ...
        card.addEventListener('mousemove', (e) => {
            ...
            // Apply rich 3D transform
            card.style.transform = \	ranslateY(-5px) rotateX(\deg) rotateY(\deg) scale3d(1.02, 1.02, 1.02)\;
            card.style.boxShadow = '0 15px 35px rgba(255, 87, 34, 0.15), 0 5px 15px rgba(0,0,0,0.05)';
            
            // Move glare
            const glareX = (x / rect.width) * 100;
            const glareY = (y / rect.height) * 100;
            glare.style.background = \adial-gradient(circle at \% \%, rgba(255,255,255,0.4) 0%, transparent 60%)\;
